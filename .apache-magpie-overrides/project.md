...
# `Apache Fineract` — project manifest

This is the **project configuration** for `Apache Fineract`.
...
| `short_name` | `Fineract` |
| `product_family_url` | `https://fineract.apache.org/` |
...
| `upstream_repo` | `apache/fineract` | Public codebase where fixes land |
| `upstream_repo_url` | `https://github.com/apache/fineract` | |
| `upstream_default_branch` | `develop` | Upstream's default branch — what `<default-branch>` resolves to. Distinct from `tracker_default_branch` |
| `upstream_agents_md_url` | TODO: `https://github.com/<upstream>/blob/main/AGENTS.md` | Conventions this repo mirrors |
| `upstream_contributing_docs_url` | `https://github.com/apache/fineract/blob/develop/CONTRIBUTING.md` | |
| `upstream_genai_disclosure_anchor` | TODO: URL + anchor for the project's Gen-AI disclosure guideline | |
| `upstream_security_policy_url` | `https://github.com/apache/fineract/security/policy` | |
...
| Key | Value | Notes |
|---|---|---|
| `security_list` | `security@fineract.apache.org` | Inbound reports; **not** publicly archived (auto-sourced default) |
| `private_list` | `private@fineract.apache.org` | PMC escalation; **not** publicly archived (auto-sourced default) |
| `users_list` | `users@fineract.apache.org` | Public advisories end up here; publicly archived (auto-sourced from `.asf.yaml` or default) |
| `dev_list` | `dev@fineract.apache.org` | Release `[RESULT][VOTE]` threads; publicly archived (auto-sourced from `.asf.yaml` or default) |
| `announce_list` | `announce@apache.org` | Cross-project announcement list; publicly archived |
| `commits_list` | `commits@fineract.apache.org` | Publicly archived (auto-sourced from `.asf.yaml`) |
